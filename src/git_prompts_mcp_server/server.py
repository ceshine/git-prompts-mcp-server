...
async def run(repository: Path, excludes: list[str] = [], json_format: bool = True):
    ...
    async def get_prompt(name: str, arguments: dict[str, str] | None = None) -> types.GetPromptResult:
        ...
        if name == "generate-pr-desc":
            ...
            source_commit = repo.commit(arguments.get("ancestor"))
            target_commit = repo.commit(arguments.get("HEAD"))
            ...
            for exclude_pattern in excludes:
                diff_results = [
                    item
                    for item in diff_results
                    if not fnmatch(item.a_path or "", exclude_pattern)
                    and not fnmatch(item.b_path or "", exclude_pattern)
                ]
                    ...
                    messages=[
                        types.PromptMessage(
                            ...
                            content=types.TextContent(
                                type="text",
                                text=_format_diff_results_as_json(diff_results)
                                + "\n\n Above is the diff results for a pull request. Please create descriptions for this pull request.",
                            ),
                        )
                    ]
                )
